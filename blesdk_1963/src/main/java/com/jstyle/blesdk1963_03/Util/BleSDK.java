package com.jstyle.blesdk1963_03.Util;


import android.text.TextUtils;
import android.util.Log;

import com.jstyle.blesdk1963_03.callback.DataListener1963;
import com.jstyle.blesdk1963_03.constant.BleConst;
import com.jstyle.blesdk1963_03.constant.DeviceConst;
import com.jstyle.blesdk1963_03.constant.DeviceKey;

import com.jstyle.blesdk1963_03.model.MyAutomaticHRMonitoring;
import com.jstyle.blesdk1963_03.model.Clock;
import com.jstyle.blesdk1963_03.model.MyDeviceInfo;
import com.jstyle.blesdk1963_03.model.MyPersonalInfo;
import com.jstyle.blesdk1963_03.model.Notifier;
import com.jstyle.blesdk1963_03.model.MyDeviceTime;
import com.jstyle.blesdk1963_03.model.MySedentaryReminder;
import com.jstyle.blesdk1963_03.model.SportPeriod;
import com.jstyle.blesdk1963_03.model.StepModel;
import com.jstyle.blesdk1963_03.model.WeatherData;
...
import static com.jstyle.blesdk1963_03.Util.ResolveUtil.bcd2String;
import static com.jstyle.blesdk1963_03.Util.ResolveUtil.getFloat;
import static com.jstyle.blesdk1963_03.Util.ResolveUtil.getValue;
import static com.jstyle.blesdk1963_03.constant.BleConst.DeviceSendDataToAPP;
import static com.jstyle.blesdk1963_03.constant.DeviceKey.Mode;
...
public class BleSDK {
    ...
    public static void DataParsingWithData(byte[] value, DataListener1963 dataListener) {
...
/*            case DeviceConst.ReadSerialNumber:
            ...
            case  DeviceConst.HealthStatusAlertPushWithStatus:
                Map<String, Object> mapsa = new HashMap<>();
                mapsa.put(DeviceKey.DataType, BleConst.HealthPushWithTyp);
                mapsa.put(DeviceKey.End, true);
                dataListener.dataCallback(mapsa);
                break;
            case  DeviceConst. HealthStatusColorSwitchOnMainInterfaceWithMode:
                if(isreadhert){
                    Map<String,Object> bna=new HashMap<>();
                    Map<String, String> ccccc = new HashMap<>();
                    ccccc.put(Mode,getValue(value[1], 0)+"");
                    bna.put(DeviceKey.DataType,BleConst. HealthStatusColorSwitchOnMainInterfaceWithMode);
                    bna.put(DeviceKey.End, true);
                    bna.put(DeviceKey.Data, ccccc);
                    dataListener.dataCallback(bna);
                }else{
                    Map<String,Object> bna=new HashMap<>();
                    bna.put(DeviceKey.DataType,BleConst. GetHealthStatusColorSwitchOnMainInterfaceWithMode);
                    bna.put(DeviceKey.End, true);
                    dataListener.dataCallback(bna);
                }
            ...
            case DeviceConst.History_of_blood_pressure:
                dataListener.dataCallback(ResolveUtil.getBloodPressure(value));
                break;
     ...
     * @return
     */
    public static byte[] SetDistanceUnit(boolean iskm) {
        byte[] value = new byte[16];
        value[0] = DeviceConst.CMD_Set_DeviceInfo;
        value[1] =  iskm?(byte)0x81:(byte)0x80;
        crcValue(value);
        return value;
    }
    public static byte[] SetTempUnit(boolean c) {
        byte[] value = new byte[16];
        value[0] = DeviceConst.CMD_Set_DeviceInfo;
        value[4] = c?(byte)0x81:(byte)0x80;
        crcValue(value);
        return value;
    }
    ...
    public static byte[] setIsItnighttime(boolean enable) {
        byte[] value = new byte[16];
        value[0] = DeviceConst.CMD_Set_DeviceInfo;
        value[3] = enable?WristOn_Enable:WristOn_DisEnable;
        crcValue(value);
    ...
    public static byte[] setSocialDistanceSwitch(boolean enable) {
        byte[] value = new byte[16];
        value[0] = DeviceConst.CMD_Set_DeviceInfo;
        value[13] = enable?WristOn_Enable:WristOn_DisEnable;
        crcValue(value);
        return value;
    }
    public static byte[] setIsitinChinese(boolean enable) {
        byte[] value = new byte[16];
        value[0] = DeviceConst.CMD_Set_DeviceInfo;
        value[14] = enable?WristOn_Enable:WristOn_DisEnable;
        crcValue(value);
        return value;
    }
    ...
    public static byte[] HealthPushWithType(int mode,String strPushTitle,String strPushContent) {
        byte[] infoValue = TextUtils.isEmpty(strPushContent) ? new byte[1] : getInfoValue(strPushContent, 60);
        byte[] titleValue = TextUtils.isEmpty(strPushTitle) ? new byte[1] : getInfoValue(strPushTitle, 60);
        byte[] value = new byte[124];
        value[0] =DeviceConst.HealthStatusAlertPushWithStatus;
        switch (mode){
            case 0:
                value[1] = (byte) 0x00;
                break;
        ...
        value[2] = (byte) titleValue.length;
        System.arraycopy( titleValue, 0, value, 3, titleValue.length);
        value[63] = (byte)  infoValue.length;
        System.arraycopy(infoValue, 0, value, 64, infoValue.length);
        return  value;
    }

    private static boolean isreadhert=false;
    public static byte[] HealthStatusColorSwitchOnMainInterfaceWithMode(int mode) {
        byte[] value = new byte[16];
        if(mode==99){isreadhert=true;}else {isreadhert=false;}
        value[0] = DeviceConst.HealthStatusColorSwitchOnMainInterfaceWithMode;
        if(mode==99){
            value[1] = (byte)0x99;
        }else{
            switch (mode){
   ...
   /* public static byte[] SetBaseHeartRate(int BaseHeartRate) {
    ...
    public static byte[] SetAutomaticHRMonitoring(MyAutomaticHRMonitoring autoHeart) {
        ...
        value[9] = (byte) autoHeart.getType();// 01 心率自动检测 02 血氧自动检测 03 体温自动检测 04 血压自动检测
        crcValue(value);
        return value;
    }


    public static byte[] GetAutomaticHRMonitoring(int type) {
        byte[] value = new byte[16];
   ...
   /* public static byte[] StartHeartRateMode() {
    ...
    public static byte[] Bloodpressure(int Number) {
        byte[] value = new byte[16];
        value[0] = DeviceConst.History_of_blood_pressure;
        value[1] = (byte) Number;
        crcValue(value);
        return value;
    }
    ...
    public static byte[] SetDeviceInfo(MyDeviceInfo deviceBaseParameter) {
        ...
        value[5] = (byte) (deviceBaseParameter.isNight_mode() ? 0x81 : 0x80);
        value[6] = (byte )0x80;
